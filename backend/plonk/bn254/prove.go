...
func (s *instance) solveConstraints() error {
	...
	if s.opt.ProvidedSolution != nil {
		// Use provided solution instead of solving
		var ok bool
		solution, ok = s.opt.ProvidedSolution.(*cs.SparseR1CSSolution)
		if !ok {
			return fmt.Errorf("provided solution must be *cs.SparseR1CSSolution, got %T", s.opt.ProvidedSolution)
		}
	} else {
